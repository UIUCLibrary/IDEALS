...
/**
 * This StatsBitstreamReader extends the normal DSpace XMLUI BitstreamReader
 * at org.dspace.app.xmlui.cocoon.BitstreamReader.  It adds recording
 * of Statistics using the U of Rochester Statistics AddOn.
 * 
 ...
 * @author Bill Ingram (U of Illinois)
 */

public class StatsBitstreamReader extends AbstractReader implements Recyclable
{
    private static Logger log = Logger.getLogger(StatsBitstreamReader.class);
            ...
            IOException
    {
        ...
        try
        {
            ...
            /** IDEALS: For Download Statistics **/
            //  Update the download count, assuming we have a valid bitstream
            if (this.bitstreamSize>0 && item!=null && item.isArchived()==true)
            {
            	//Check bitstream's bundles
            	Bundle[] bundles = bitstream.getBundles();
            	boolean countDownload = true;
                for (Bundle bundle : bundles)
                {
                    if(bundle.getName().equals(Constants.CONTENT_BUNDLE_NAME))
                    {
                        //Check if this is an image embedded in a webpage:
                        // We *don't* want to count downloads of images which
                        // are just referenced by a primary HTML file.
                        if(this.bitstreamMimeType.startsWith("image/"))
                        {
                            if(bundle.getPrimaryBitstreamID()>=0)
                            {
                                Bitstream primaryBitstream = Bitstream.find(context, bundle.getPrimaryBitstreamID());
                                if(primaryBitstream!=null && primaryBitstream.getFormat().getMIMEType().equals("text/html"))
                                {
                                  countDownload=false;
                                }
                            }
                    ...
                    // We *don't* want to count downloads of
                    // Thumbnails, Plain Text (used for indexing),
                    // Metadata files or Licenses
                    else if(bundle.getName().equals("THUMBNAIL") ||
                       bundle.getName().equals("BRANDED_PREVIEW") ||
                	   bundle.getName().equals("TEXT") ||
                	   bundle.getName().equals(Constants.LICENSE_BUNDLE_NAME) ||
                	   bundle.getName().equals(Constants.METADATA_BUNDLE_NAME))
                	{
                		countDownload=false;
                    }

                    if(!countDownload) break;
                }//end for each bundle
            	
            	if(countDownload && !isSpider)
            	{	
	            	//Despite the name, updateCount only counts
	            	//*one* download per day from a given IP address.
	        		StatsDAO statsDAO = new StatsDAO();
	        		statsDAO.updateCount(item.getID(), bitstream.getID(), request.getRemoteAddr());
            	}	
            }
            /*** End For Download Statistics ***/
